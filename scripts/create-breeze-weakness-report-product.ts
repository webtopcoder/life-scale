...
const PRODUCT_DISPLAY_NAME = 'Weakness Report';
const PRODUCT_DESCRIPTION = 'One-time weakness report add-on';
...
async function main(): Promise<void> {
  ...
  console.log('\nBreeze regular product setup complete (Weakness Report).\n');
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log(`BREEZE_WEAKNESS_REPORT_PRODUCT_ID=${productId}`);
  // eslint-disable-next-line no-console
  console.log(`BREEZE_WEAKNESS_REPORT_PRICE_ID=${priceId}`);
}

main().catch((error: unknown) => {
  // eslint-disable-next-line no-console
  console.error('Failed to create Breeze Weakness Report product/price:', error);
  process.exit(1);
});
