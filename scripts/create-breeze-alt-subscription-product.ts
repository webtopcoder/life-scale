...
async function main(): Promise<void> {
  ...
  console.log('\nBreeze alt subscription setup complete.\n');
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log(`BREEZE_IQ_ALT_SUBSCRIPTION_PRODUCT_ID=${productId}`);
  // eslint-disable-next-line no-console
...
main().catch((error: unknown) => {
  // eslint-disable-next-line no-console
  console.error('Failed to create Breeze alt subscription product/price:', error);
  process.exit(1);
});
