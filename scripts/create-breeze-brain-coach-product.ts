...
const PRODUCT_DISPLAY_NAME = 'Brain Coach';
const PRODUCT_DESCRIPTION = 'One-time Brain Coach add-on';
...
async function main(): Promise<void> {
  ...
  console.log('\nBreeze regular product setup complete (Brain Coach).\n');
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log(`BREEZE_BRAIN_COACH_PRODUCT_ID=${productId}`);
  // eslint-disable-next-line no-console
  console.log(`BREEZE_BRAIN_COACH_PRICE_ID=${priceId}`);
}

main().catch((error: unknown) => {
  // eslint-disable-next-line no-console
  console.error('Failed to create Breeze Brain Coach product/price:', error);
  process.exit(1);
});
