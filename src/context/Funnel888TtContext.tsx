...
/**
 * Isolated funnel context for the /onboarding-888-tt flow.
...
const STATE_KEY = 'lifescale_888_tt_state';
const SESSION_KEY = 'lifescale_888_tt_session_id';
...
export const FUNNEL_888_TT_FLOW_ID = 'f0000000-0000-0000-0000-000000000007';

export type Funnel888TtState = FunnelState & {
  memoryIntroComplete: boolean;
  analysisProgress: number;
  analysisResponses: Record<string, string>;
};

const initialState: Funnel888TtState = {
  ...
  flowId: FUNNEL_888_TT_FLOW_ID, selectedPlanId: null,
  memoryIntroComplete: false, analysisProgress: 0, analysisResponses: {},
};

export type Funnel888TtAction =
...
function loadState(): Funnel888TtState {
  try {
    const saved = localStorage.getItem(STATE_KEY);
    return saved ? { ...initialState, ...(JSON.parse(saved) as Partial<Funnel888TtState>), flowId: FUNNEL_888_TT_FLOW_ID } : initialState;
  } catch { return initialState; }
}

function reducer(state: Funnel888TtState, action: Funnel888TtAction): Funnel888TtState {
  switch (action.type) {
...
export function get888TtSessionId() {
  let id = localStorage.getItem(SESSION_KEY);
  if (!id) { id = makeId(); localStorage.setItem(SESSION_KEY, id); }
  return id;
}

type ContextValue = { state: Funnel888TtState; dispatch: React.Dispatch<Funnel888TtAction>; goToStage: (stage: FunnelStage) => void };
const Funnel888TtContext = createContext<ContextValue | null>(null);

export function Funnel888TtProvider({ children }: { children: React.ReactNode }) {
  ...
  const wrappedDispatch = useCallback((action: Funnel888TtAction) => {
    dispatch(action);
  ...
  return <Funnel888TtContext.Provider value={value}>{children}</Funnel888TtContext.Provider>;
}

export function useFunnel888Tt() {
  const context = useContext(Funnel888TtContext);
  if (!context) throw new Error('useFunnel888Tt must be used inside Funnel888TtProvider');
  return context;
}
