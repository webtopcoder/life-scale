...
import { Check, Clock, Shield, ShieldCheck, Star, ArrowRight, Lock, MapPin, Brain, FileText, Lightbulb, TrendingUp, Eye, ChevronRight, CreditCard, Gift } from 'lucide-react';
...
import { CheckoutTimerBar } from '@/components/CheckoutTimerBar';
...
/** When not explicitly `false`, show email gate on checkout if funnel has no email yet. Set `VITE_CHECKOUT_EMAIL_OVERLAY=false` to disable. */
...
const CheckoutPage = () => {
  ...
  const startedCheckoutWithoutEmailRef = useRef(!state.email.trim());
  ...
  const [shouldInitCheckout, setShouldInitCheckout] = useState(true);

  const showCheckoutEmailGate = CHECKOUT_EMAIL_OVERLAY_ENABLED && !state.email.trim();
  ...
  useEffect(() => {
    if (shouldInitCheckout) handlePaymentStarted();
  }, [shouldInitCheckout, handlePaymentStarted]);
  ...
  return (
    <div className={cn('min-h-screen bg-background relative', showCheckoutEmailGate && 'overflow-hidden')}>
      ...
      <section className="max-w-5xl mx-auto px-4 py-6 md:py-8 overflow-x-hidden bg-card rounded-xl">
        ...
        <div className="max-w-lg mx-auto space-y-6">
          ...
          <div className="space-y-6 min-w-0 overflow-hidden">
            ...
            <Card
              id="payment-section"
              className={`overflow-hidden scroll-mt-[92px] ${isAltFlow
                ? 'rounded-2xl border-2 border-[#4A7CFF] shadow-none bg-white'
                : 'border border-border shadow-[0_8px_30px_-4px_hsl(var(--foreground)/0.15)]'
                }`}
            >
              ...
              <CardContent className="p-4 sm:p-6 space-y-5">
                {isAltFlow ? (
                  ...
                  <div className="flex items-baseline justify-between px-1">
                    ...
                    {/* <span className="text-xl font-bold text-foreground">{formatUsdLabel(IQ_TRIAL_DUE_CENTS)}</span> */}
                    <span className="text-xl font-bold text-foreground">{formatUsdLabel(IQ_TRIAL_DUE_CENTS)}</span>
                  </div>
                )}
                ...
                {/* Security badges (default flow only) */}
                {!isAltFlow && (
