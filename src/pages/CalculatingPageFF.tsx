...
const CalculatingPageFF = () => {
  const { state, dispatch, goToStage } = useFunnel();
  const { t, formatNumber } = useFFIntl();
  ...
  const handleWarningDismiss = () => {
    ...
    trackEvent(undefined, EVENTS.FUNNEL_STEP_COMPLETED, { step: 'calculating' });
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="max-w-sm w-full space-y-8 text-center relative z-10">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-1">
          <h2 className="text-2xl font-bold text-foreground">{t('Calculating Your Results')}</h2>
          <p className="text-sm text-muted-foreground">{t('Please wait while we analyze your performance…')}</p>
        </motion.div>
        ...
        <div className="space-y-3">
          {STEPS.map((step, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: i <= currentStep ? 1 : 0.3, x: 0 }} transition={{ delay: i * 0.12, type: 'spring', stiffness: 200, damping: 20 }} className="flex items-center gap-3 text-left">
              ...
              <span className={`text-sm ${i <= currentStep ? 'text-foreground' : 'text-muted-foreground'}`}>{t(step)}</span>
            </motion.div>
          ))}
        </div>
        ...
        <motion.p className="text-xs text-muted-foreground" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
          {t('{count} records analyzed', { count: formatNumber(recordsAnalyzed) })}
        </motion.p>
      </div>
        ...
        {showPuzzlePopup && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <motion.div initial={{ opacity: 0, scale: 0.85, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} transition={{ type: 'spring', stiffness: 260, damping: 22, delay: 0.1 }} className="bg-card rounded-2xl shadow-xl max-w-sm w-full p-6 space-y-5 text-center">
              ...
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.4 }} className="text-lg font-semibold text-foreground">{t('Do you enjoy challenging your mind?')}</motion.p>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, duration: 0.4 }} className="flex gap-3">
                <Button onClick={handlePuzzleDismiss} className="flex-1 h-12 text-base font-bold rounded-xl" style={{ background: 'var(--gradient-primary)' }}>{t('No')}</Button>
                <Button onClick={handlePuzzleDismiss} className="flex-1 h-12 text-base font-bold rounded-xl" style={{ background: 'var(--gradient-primary)' }}>{t('Yes')}</Button>
              </motion.div>
            </motion.div>
          </motion.div>
        )}
        ...
        {showSecondPopup && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <motion.div initial={{ opacity: 0, scale: 0.85, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} transition={{ type: 'spring', stiffness: 260, damping: 22, delay: 0.1 }} className="bg-card rounded-2xl shadow-xl max-w-sm w-full p-6 space-y-5 text-center">
              ...
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.4 }} className="text-lg font-semibold text-foreground">{t('Are you a curious person?')}</motion.p>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, duration: 0.4 }} className="flex gap-3">
                <Button onClick={handleSecondDismiss} className="flex-1 h-12 text-base font-bold rounded-xl" style={{ background: 'var(--gradient-primary)' }}>{t('No')}</Button>
                <Button onClick={handleSecondDismiss} className="flex-1 h-12 text-base font-bold rounded-xl" style={{ background: 'var(--gradient-primary)' }}>{t('Yes')}</Button>
              </motion.div>
            </motion.div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showWarning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <motion.div initial={{ opacity: 0, scale: 0.85, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} transition={{ type: 'spring', stiffness: 260, damping: 22, delay: 0.1 }} className="bg-card rounded-2xl shadow-xl max-w-sm w-full p-6 space-y-5 text-center">
              <motion.h3 initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.4 }} className="text-xl font-bold text-primary tracking-wide uppercase">{t('Warning')}</motion.h3>
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45, duration: 0.4 }} className="text-foreground text-[15px] leading-relaxed">{t('Your results may surprise you. IQ report reveals hidden strengths and growth areas — a powerful tool for unlocking your potential.')}</motion.p>
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.55, duration: 0.4 }} className="flex items-center gap-3 bg-muted/60 rounded-xl px-4 py-3 text-left">
                <Lock className="w-5 h-5 text-primary flex-shrink-0" />
                <p className="text-sm text-muted-foreground">{t('Your results remain confidential and personally yours.')}</p>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65, duration: 0.4 }}>
                <Button onClick={handleWarningDismiss} className="w-full h-12 text-base font-bold uppercase tracking-wider rounded-xl bg-[hsl(var(--cta))] hover:bg-[hsl(var(--cta-hover))] text-white">{t('I Understand')}</Button>
              </motion.div>
            </motion.div>
          </motion.div>
        )}
...
export default CalculatingPageFF;
