...
import { getQuestion, getTotalQuestions, preloadQuestions } from '@/engine/datasetLoader';
  ...
  const handleAnswer = async (optionIndex: number) => {
    ...
    if (question.type !== 'likert' && question.type !== 'personal') {
      if (hasVisualPuzzle) {
        ...
        // Dev mode: use local correctAnswer directly
                  ...
                  <div className="flex flex-col gap-3">
                    {question.options.map((option, i) => (
                      <button
                        ...
                        type="button"
                        {...pointerSafeActivation(() => handleAnswer(i))}
                        style={touchFeedbackStyle(selectedOption === i ? 'hsl(var(--primary) / 0.15)' : 'hsl(var(--primary) / 0.06)')}
                        className={`w-full px-5 py-3.5 text-sm font-medium text-left rounded-xl transition-all active:scale-[0.98] text-foreground/80 touch-no-hover ${selectedOption === i ? 'bg-primary/15 ring-1 ring-primary/60' : 'bg-primary/[0.06] hover:bg-primary/10'}`}
                      >
                        {option}
                      </button>
                    ))}
...
export default AssessmentPageDev;
