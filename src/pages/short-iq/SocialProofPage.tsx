...
const REVIEWS = [
  { name: 'Cara Mitchell', date: 'January 10, 2026', avatar: avatarCara, title: '"It helped me understand myself better!"',
  ...
  { name: 'James Okafor', date: 'February 3, 2026', avatar: avatarJames, title: '"Surprisingly accurate results"',
  ...
  { name: 'Sophie Laurent', date: 'January 28, 2026', avatar: avatarSophie, title: '"A real eye-opener"',
  ...
  { name: 'Raj Patel', date: 'February 8, 2026', avatar: avatarRaj, title: '"Better than I expected"',
  ...
  { name: 'Mei Chen', date: 'January 19, 2026', avatar: avatarMei, title: '"Fascinating cognitive breakdown"',
...
const SocialProofPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F5FCFF] flex flex-col items-center">
      <div className="flex-1 overflow-y-auto w-full flex flex-col items-center px-4 pt-4 pb-28">
        <div className="max-w-md w-full space-y-8">
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
            ...
            <span className="text-primary">50 Million+ people</span> have discovered their IQ score
          </motion.h1>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }}
            ...
            <p className="font-semibold text-foreground text-[15px]">{review.title}</p>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.4 }}>
            <motion.div animate={{ scale: [1, 1.03, 1] }} transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}>
              <Button onClick={() => {
                trackEvent(posthog, EVENTS.FUNNEL_STEP_COMPLETED ?? 'funnel_step_completed', { step: 'social_proof' });
                dispatch({ type: 'SET_STAGE', stage: 'calculating' });
              }}
                className="w-full h-12 text-base font-bold rounded-xl bg-[#0088D1] hover:bg-[#0088D1]/90 text-white shadow-[0_0_20px_rgba(0,136,209,0.4)]">
                Continue
              </Button>
            </motion.div>
          </motion.div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.5 }} className="space-y-3">
            <div className="flex items-center gap-3">
              <Users className="w-5 h-5 text-muted-foreground flex-shrink-0" />
              <p className="text-sm text-foreground">
                <span className="font-bold">{userCount.toLocaleString()} users</span> took their IQ test today.
              </p>
            </div>
            <div className="flex items-center gap-3">
              <MessageCircle className="w-5 h-5 text-muted-foreground flex-shrink-0" />
              <p className="text-sm text-foreground">
                Trusted by over <span className="font-bold">50 million</span> people.{' '}
                {[...Array(5)].map((_, i) => (<span key={i} className="text-warning">★</span>))}
              </p>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="w-full overflow-hidden pt-2">
            <div className="relative"><LogoBanner /></div>
          </motion.div>
        </div>
      </div>
    </div>
