...
const AssessmentPageFF = () => {
  const { state, dispatch } = useFunnel();
  const { t, localizeQuestion } = useFFIntl();
  const theme = getFunnelTheme(state.flowId);
  const questionStartRef = useRef(Date.now());
  const [direction, setDirection] = useState(1);

  const total = getTotalQuestions();
  const sourceQuestion = getQuestion(state.questionIndex);
  const question = sourceQuestion ? localizeQuestion(sourceQuestion) : undefined;
  ...
  const handleAnswer = (optionIndex: number) => {
    ...
    trackEvent(undefined, EVENTS.ASSESSMENT_QUESTION_ANSWERED, {
  ...
  const likertLabels = ["Strongly Disagree", "Disagree", "Neutral", "Agree", "Strongly Agree"].map((label) => t(label));

  return (
    <div data-testid="assessment-root" className="h-screen bg-white flex flex-col overflow-hidden">
      {/* Header */}
          ...
          {skippedIndices.map((idx) => {
            ...
            return (
              <div
                ...
                style={{ left: `${pos}%` }}
                title={t('Question {number} skipped', { number: idx + 1 })}
              />
            );
          })}
        </div>
                ...
                {question.type === 'likert' ? (
                  ...
                  <>
                    ...
                    <div className="md:hidden flex-shrink-0">
                       <p className="text-base font-light text-foreground text-center mb-4">{t('Choose your answer:')}</p>
                    ...
                    <div className="hidden md:flex md:flex-row md:items-center md:justify-center md:gap-6 flex-shrink-0">
                      ...
                      <div className="max-w-[320px] w-full">
                         <p className="text-base font-light text-foreground text-center mb-3">{t('Choose your answer:')}</p>
      ...
      <div className="shrink-0 bg-card/90 backdrop-blur-sm border-t border-border/60 px-4 py-1.5">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          ...
          <span data-testid="assessment-progress" className="text-xs text-muted-foreground">
            {t('{current} of {total}', { current: state.questionIndex + 1, total })}
          </span>
...
export default AssessmentPageFF;
